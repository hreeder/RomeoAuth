...
#			pass
	...
	def generatemessage(self, sender, to, message):
		...
		result = "\n" + message
		result = result+"\n\n"
		result = result+"== %s to %s @ %s UTC/EVE ==" % (sender, to, strftime("%Y/%m/%d %H:%M", gmtime()))
		return result
	...
	def sendmessage(self, tojids, message):
		jidparams={}
		jidparams['jid'] = self.username+"@"+self.domain+"/pingbot"
		jidparams['password'] = self.passwd
		...
		total = 0
		for tojid in tojids:
			id=cl.send(xmpp.protocol.Message(tojid,message,'chat'))
			#id=cl.send(xmpp.protocol.Message(tojid,message))
			total = total+1
		cl.disconnect()
		return total
